        ...
        foreach ($request->delivery_from_date as $index => $from_date) {
            ...
            if ($from_date || $from_time || $to_date || $to_time) {
                if (!$from_date || !$from_time || !$to_date || !$to_time) {
                    ...
                    try {
                        // 変更①: 日付と時間のフォーマットを 'Y-m-d H:i' に変更
                        $formattedFromDate = Carbon::createFromFormat('Y-m-d H:i', $from_date . ' ' . $from_time);
                        $formattedToDate = Carbon::createFromFormat('Y-m-d H:i', $to_date . ' ' . $to_time);
            ...
            foreach ($request->delivery_from_date as $index => $from_date) {
                ...
                if ($from_date && $from_time && $to_date && $to_time) {
                    try {
                        ...
                        $formattedFromTime = $from_time;
                        $formattedToDate = $to_date;
                        $formattedToTime = $to_time;
